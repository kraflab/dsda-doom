...
boolean P_TryWalk(mobj_t * actor)
{
    ...
    actor->movecount = P_Random(pr_hexen) & 15;
    return (true);
}
...
// try other directions
    if (P_Random(pr_hexen) > 200 || abs(deltay) > abs(deltax))
    {
        tdir = d[1];
        d[1] = d[2];
        d[2] = tdir;
...
/* there is no direct path to the player, so pick another direction */
    ...
    if (P_Random(pr_hexen) & 1)         /*randomly determine direction of search */
    {
...
//
    ...
    {
        if (actor->type == HEXEN_MT_BISHOP && P_Random(pr_hexen) < 128)
        {
...
void A_MinotaurAtk1(mobj_t * actor)
{
    ...
    S_StartSound(actor, hexen_sfx_maulator_hammer_swing);
    if (P_CheckMeleeRange(actor))
    {
...
void A_MinotaurCharge(mobj_t * actor)
{
    ...
    if (actor->args[4] > 0)
    {
        puff = P_SpawnMobj(actor->x, actor->y, actor->z, HEXEN_MT_PUNCHPUFF);
...
void A_MinotaurAtk2(mobj_t * actor)
{
    ...
    mo = P_SpawnMissile(actor, actor->target, HEXEN_MT_MNTRFX1);
    if (mo)
    {
        //S_StartSound(mo, sfx_minat2);
        momz = mo->momz;
        angle = mo->angle;
        P_SpawnMissileAngle(actor, HEXEN_MT_MNTRFX1, angle - (ANG45 / 8), momz);
        P_SpawnMissileAngle(actor, HEXEN_MT_MNTRFX1, angle + (ANG45 / 8), momz);
        P_SpawnMissileAngle(actor, HEXEN_MT_MNTRFX1, angle - (ANG45 / 16), momz);
        P_SpawnMissileAngle(actor, HEXEN_MT_MNTRFX1, angle + (ANG45 / 16), momz);
    }
}
...
void A_MinotaurAtk3(mobj_t * actor)
{
    ...
    {
        mo = P_SpawnMissile(actor, actor->target, HEXEN_MT_MNTRFX2);
        if (mo != NULL)
        {
            S_StartSound(mo, hexen_sfx_maulator_hammer_hit);
        }
    }
...
/*	switch(actor->type)
	{
		...
		case HEXEN_MT_MUMMYLEADERGHOST:
			P_DropItem(actor, HEXEN_MT_AMGWNDWIMPY, 3, 84);
			break;
		default:
			break;
	}
...
void A_Explode(mobj_t * actor)
{
    ...
    if (actor->z <= actor->floorz + (distance << FRACBITS)
        && actor->type != HEXEN_MT_POISONCLOUD)
    {
        P_HitFloor(actor);
    }
}
...
void A_SkullPop(mobj_t * actor)
{
    ...
    mo->momz = FRACUNIT * 2 + (P_Random(pr_hexen) << 6);
