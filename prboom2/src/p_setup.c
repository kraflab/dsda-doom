...
static int C_DECL dicmp_sprite_by_pos(const void *a, const void *b)
{
  ...
  return res ? res : m2->index - m1->index;
}
...
static void P_PostProcessThings(int mobjcount, mobj_t **mobjlist)
{
  ...
  if (V_IsOpenGLMode())
  {
    ...
    {
      for (i = 1; i < mobjcount; ++i)
      {
        mobj_t *m_ref;
        uint64_t counter = 0;

        for (m_ref = mobjlist[i - 1]; i < mobjcount && GETXY(mobjlist[i]) == GETXY(m_ref); ++i)
          mobjlist[i]->flags |= (((++counter) & 0xf) << MF_FOREGROUND_SHIFT);
      }
    }
  }
