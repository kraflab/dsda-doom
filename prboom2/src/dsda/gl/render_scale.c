...
void dsda_SetRenderViewportParams() {
    ...
    lprintf(LO_INFO, "dsda_SetRenderViewportParams: Viewport %dx%d\n", gl_viewport_width, gl_viewport_height);
    lprintf(LO_INFO, "dsda_SetRenderViewportParams: Scene %dx%d\n", gl_scene_width, gl_scene_height);
    lprintf(LO_INFO, "dsda_SetRenderViewportParams: Scene Offset %d & %d\n", gl_scene_offset_x, gl_scene_offset_y);
    lprintf(LO_INFO, "dsda_SetRenderViewportParams: StatusBar Height %d\n", gl_statusbar_height);
}
...
void dsda_UpdateStatusBarVisible() {
    ...
    if (saved_visible != current_visible) {
        gl_statusbar_height = (int)(gl_scale_y * (float)ST_SCALED_HEIGHT) * (viewheight != SCREENHEIGHT);
        gl_scene_height = gl_viewport_height - gl_statusbar_height - (gl_scene_offset_y * 2);
        lprintf(LO_INFO, "dsda_UpdateStatusBarVisible: StatusBar Height %d\n", gl_statusbar_height);
    }
}
