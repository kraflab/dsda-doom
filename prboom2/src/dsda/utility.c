                                    ...
                                    dsda_strview_t* after)
{
  ...
  for (cur = sv->string, size = sv->size; size; ++cur, --size)
  {
    ...
    {
      if (before)
        dsda_StrViewInit(before, sv->string, sv->size - size + 1);
      if (after)
        dsda_StrViewInit(after, cur + 1, size - 1);
      return true;
    }
  }

  if (before)
    dsda_StrViewInit(before, sv->string, sv->size);
  if (after)
    dsda_StrViewInit(after, NULL, 0);
  return false;
}
                                     ...
                                     dsda_strview_t* after)
{
  ...
  for (cur = sv->string, size = sv->size; size; ++cur, --size)
  {
    ...
    {
      if (before)
        dsda_StrViewInit(before, sv->string, sv->size - size);
      if (after)
        dsda_StrViewInit(after, cur, size);
      return true;
    }
  }
...
// Removes any character in the C string `chars` from the start of `sv`.
void dsda_StrViewTrimStartCStr(const dsda_strview_t* sv, const char* chars, dsda_strview_t* trim)
{
  ...
  dsda_StrViewInit(trim, cur, size);
}
